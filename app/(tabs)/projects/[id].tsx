    ...
    return (
        <SafeAreaView className="flex-1 bg-gray-50 dark:bg-[#09090b]">
            <Stack.Screen options={{
                headerStyle: { backgroundColor: colorScheme === 'dark' ? '#09090b' : '#f9fafb' },
                ...
                headerRight: () => (
                    <Pressable>
                        <Ionicons name="settings" size={22} color={colorScheme === 'dark' ? "rgba(255,255,255,0.6)" : "rgba(0,0,0,0.6)"} />
                    </Pressable>
                )
            }} />
            ...
            <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
                {/* Header Section */}
                <View className="flex-row items-center gap-4 px-6 py-6 border-b border-black/5 dark:border-white/5">
                    <View className={`w-14 h-14 rounded-2xl items-center justify-center border border-gray-200 dark:border-border ${iconBg}`}>
                        <Ionicons name={project.icon as any} size={28} color={iconColor} />
                    </View>
                    <View className="flex-1">
                        <Text className="text-black dark:text-white text-[22px] font-bold tracking-tight mb-1">{project.name}</Text>
                        <Text className="text-gray-500 dark:text-white/40 font-mono text-[11px] tracking-widest">ID: {project.id}</Text>
                    </View>
                    <Pressable
                        onPress={() => setSettingsVisible(true)}
                        className="w-10 h-10 items-center justify-center rounded-full bg-black/5 dark:bg-white/5"
                    >
                        <Ionicons name="settings-outline" size={20} color={colorScheme === 'dark' ? "rgba(255,255,255,0.8)" : "rgba(0,0,0,0.8)"} />
                    </Pressable>
                </View>

                {/* Details & Status Card */}
                <View className="px-5 py-6">
                    <View className="bg-white dark:bg-[#18181b] border border-gray-200 dark:border-[#27272a] rounded-2xl p-5 mb-8">
                        <View className="flex-row items-center justify-between mb-5">
                            <Text className="text-black dark:text-white font-bold text-[15px]">Details & Status</Text>
                            <Ionicons name="chevron-up" size={20} color={colorScheme === 'dark' ? "rgba(255,255,255,0.5)" : "rgba(0,0,0,0.5)"} />
                        </View>
                        ...
                        <View className="flex-row items-center gap-3 mb-4">
                            ...
                            <Text className="text-gray-300 dark:text-white/30 text-[10px] font-black">•</Text>
                            <Text className="text-gray-600 dark:text-white/50 text-[13px] font-medium">Uptime: {project.uptime}</Text>
                        </View>

                        <Text className="text-gray-700 dark:text-white/70 text-[13px] leading-[22px] mb-6 pr-4">
                            {project.description}
                        </Text>

                        <View className="flex-row gap-3">
                            <View className="flex-1 bg-gray-50 dark:bg-[#09090b] border border-gray-200 dark:border-[#27272a] rounded-xl p-4">
                                <Text className="text-gray-500 dark:text-white/40 text-[10px] font-bold uppercase tracking-widest mb-2">Alerts Today</Text>
                                <Text className="text-black dark:text-white text-2xl font-medium tracking-tight">24</Text>
                            </View>
                            <View className="flex-1 bg-gray-50 dark:bg-[#09090b] border border-gray-200 dark:border-[#27272a] rounded-xl p-4">
                                <Text className="text-gray-500 dark:text-white/40 text-[10px] font-bold uppercase tracking-widest mb-2">Error Rate</Text>
                    ...
                    <View className="flex-row items-center justify-between mb-4 px-1">
                        <Text className="text-black dark:text-white font-bold text-[15px]">Recent Notifications</Text>
                        <Pressable>
                                ...
                                <View key={notif.id} className={`flex-row gap-4 py-4 ${index !== displayNotifications.length - 1 ? 'border-b border-gray-200 dark:border-white/5' : ''}`}>
                                    ...
                                    <View className="flex-1">
                                        <View className="flex-row items-center justify-between mb-1.5">
                                            ...
                                            <Text className="text-gray-500 dark:text-white/40 font-mono text-[10px]">{notif.timestamp}</Text>
                                        </View>
                                        <Text className="text-black dark:text-white font-medium text-[15px] mb-1">{notif.title}</Text>
                                        <Text className="text-gray-600 dark:text-white/50 text-xs leading-5 pr-4" numberOfLines={2}>
                                            {notif.message}
                                        </Text>
                                    </View>
                                </View>
            ...
            >
                <View className="flex-1 justify-end bg-black/60">
                    ...
                    <View className="bg-white dark:bg-[#18181b] border-t border-gray-200 dark:border-[#27272a] rounded-t-3xl pt-4 pb-12 px-6 shadow-2xl z-10 w-full mb-0">
                        {/* Drag Handle */}
                        <View className="w-12 h-1.5 bg-black/10 dark:bg-white/10 rounded-full self-center mb-6" />

                        <Text className="text-black dark:text-white text-xl font-bold tracking-tight mb-6">Project Settings</Text>

                        {/* Project Name Input */}
                        <View className="mb-6">
                            <Text className="text-gray-600 dark:text-white/60 text-xs font-bold uppercase tracking-widest mb-2 ml-1">Project Name</Text>
                            <TextInput
                                value={projectName}
                                onChangeText={setProjectName}
                                className="bg-gray-100 dark:bg-zinc-900 border border-gray-200 dark:border-[#27272a] rounded-xl px-4 py-3.5 text-black dark:text-white text-[15px] font-medium"
                        ...
                        {/* Status Toggle */}
                        <View className="mb-8 flex-row items-center justify-between bg-gray-100 dark:bg-zinc-900 border border-gray-200 dark:border-[#27272a] rounded-xl p-4">
                            <View className="flex-1 mr-4">
                                <Text className="text-black dark:text-white text-[15px] font-bold mb-1">Active Status</Text>
                                <Text className="text-gray-600 dark:text-white/50 text-xs leading-5">
                        ...
                        <View className="flex-row gap-3">
                            <Pressable
                                onPress={() => setSettingsVisible(false)}
                                className="flex-1 py-4 items-center justify-center bg-gray-200 dark:bg-zinc-800 rounded-xl"
                            >
                                <Text className="text-black dark:text-white font-bold text-[15px]">Cancel</Text>
                            </Pressable>
                            <Pressable
                                onPress={() => setSettingsVisible(false)}
